...
/*   Updated: 2023/04/19 20:14:20 by mkhairou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_valid_extand(char *a, int declare, char *variable)
{
	...
	else
	{
		if(a[i])
		{
			while (a[i])
			{
				if(!ft_isalpha(a[i]) && ft_isdigit(a[i]) && a[i] != '_')
				{
					if(declare)
						printf("export : not an identifier: %s\n",variable);
					return(0);
...
int		check_valid(char *a, char *variable, int declare, t_mshel *shel)
{
	...
	if(a[0] == '$')
	{
		tmp = ft_getenv(shel, a + 1);
		if(!check_valid_extand(tmp, declare, variable))
			return (0);
	}
	else
	{
...
void add_env(t_mshel *shel, char *variable)
{
	int i;
	char **var;
	char **new_env;
	...
	if(!check_valid(var[0], variable, 1, shel))
		return ;
	if(var[0][0] == '$')
		var[0] = ft_strdup(ft_getenv(shel, var[0] + 1));
	while (shel->env[i])
	{
	...
	new_env[i] = ft_strdup(ft_strjoin(var[0],ft_strjoin("=",var[1])));
	i++;
	new_env[i] = NULL;
	free_array(shel->env);
	shel->env = new_env;
...
void add_x_env(t_mshel *shel, char *variable)
{
	int i;
	char **var;
	char **new_env;
	...
	while (shel->x_env[i])
	{
		if (!ft_strncmp(shel->x_env[i], var[0], ft_strlen(var[0])) && shel->x_env[i][ft_strlen(var[0])] == '=')
		{
			free(shel->x_env[i]);
...
void	ft_export(t_mshel *shel, int cmd_index)
{
	...
	else
	{
		while (shel->cmd[cmd_index]->args[i])
		{
			...
			{
				add_env(shel, shel->cmd[cmd_index]->args[i]);
				add_x_env(shel, shel->cmd[cmd_index]->args[i]);
			}
			else
				add_x_env(shel, shel->cmd[cmd_index]->args[i]);
			i++;
